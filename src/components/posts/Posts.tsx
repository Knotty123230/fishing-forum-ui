...
const Posts: React.FC = () => {
    const [posts, setPosts] = useState<Post[]>([
        { id: 1, name: "Перший пост", description: "Опис першого поста", userId: 1, photoUrl: "public/1_preview-ozero.jpg", date: "11.01.2024" },
        { id: 2, name: "Другий пост", description: "Опис другого поста", userId: 2,photoUrl: "public/images.jpg", date: "11.01.2024" },
        { id: 3, name: "Третій пост", description: "Опис третього поста", userId: 3,photoUrl: "public/pexels-eberhard-grossgasteiger-2098405.jpg", date: "11.01.2024" }
    ]);
        ...
        <div className="posts_container">
            {posts.map((post, index) => (
                <div key={post.id} className="post">
                    ...
                    <button
                        className="post__button-likes">Лайки: {comments.filter(comment => comment.postId === post.id).reduce((total, comment) => total + comment.likes, 0)}</button>
                </div>
            ))}
        </div>
    );
